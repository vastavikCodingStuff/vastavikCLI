...
- 🛠️ **Improvements**: Obsidian dark theme (`#0B1120`/`#1E293B`) with neon cyan/green active highlights, Termux-style ExtraKeys bar with sticky CTRL/ALT, PTY `TIOCSWINSZ` resize via JNI `openpty`, PRoot commands per-distro isolation, DTO verification (`resolv.conf` 1.1.1.1/8.8.8.8) for `apt`.
...
- **Commit SHA**: <pending squashed commit SHA on main - will be replaced after PR merge>
